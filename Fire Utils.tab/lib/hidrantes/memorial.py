...
def _renderiza_bloco(console, montar_fn):
    """Roda `montar_fn()` com `output` trocado por um buffer _Memorial e
    imprime o HTML resultante no console com a mesma folha de estilo do
    memorial completo (_CSS_CONSOLE) - para blocos avulsos (resumo final
    de "Dimensionar Hidrantes", mensagens de bloqueio de verificação) que
    devem ter a mesma cara do memorial, sem precisar monta-lo inteiro."""
    global output
    doc_mem = _Memorial()
    output = doc_mem
    try:
    ...
    console.print_html(u"<style>{}</style><div class='fu-memorial'>{}</div>".format(
        _CSS_CONSOLE, doc_mem.corpo()))


def imprimir_bloqueio_velocidade(console, nome_trecho, j, limite, falhas):
    """Bloco estilizado mostrando quais diâmetros do trecho passaram do
    limite de velocidade - chamado por "Dimensionar Hidrantes" quando o
    ...
    def _montar():
        ...
        output.print_md(u"Vazão do trecho: **{:.2f} L/min**. Limite normativo: **{:.1f} m/s**.".format(
            j["Q_lmin"], limite))
        _tabela([u"DN (mm)", u"V (m/s)", u"Limite (m/s)"],
                [[u"{:.1f}".format(s["d_mm"]), u"**{:.3f}**".format(s["V"]),
                  u"{:.1f}".format(limite)] for s in falhas])
        output.print_md(
            u"**Correção necessária:** aumente o diâmetro nominal do(s) tubo(s) acima "
            u"nesse trecho (ou reduza a vazão, se possível) até a velocidade ficar dentro "
            u"do limite normativo. Ajuste o traçado no Revit e execute 'Dimensionar "
            u"Hidrantes' novamente.")
    _renderiza_bloco(console, _montar)


def imprimir_bloqueio_hidrante(console, label, p, q, p_ref_desc, trecho_desc, Pmin, Qs_lmin):
    """Bloco estilizado mostrando por que a pressão/vazão de um hidrante
    mais desfavorável não atendeu a norma - chamado por "Dimensionar
    Hidrantes" quando o dimensionamento é interrompido nessa verificação."""
    def _montar():
        output.print_md(u"---")
        output.print_md(u"## {} {} não atende a norma".format(SIM_X, label))
        _tabela([u"Grandeza", u"Obtido", u"Mínimo exigido"],
...
def imprimir_resumo_dimensionamento(console, res, valor_sistema, metodo_calculo, norma,
                                     v_max_tubo, v_max_succao, p_ref_desc,
                                     p_hd01_ref, p_hd02_ref, Pmin, Qs_lmin,
                                     eta, pot_cv, pot_kw):
    """
    Resumo estilizado mostrado ao final de "Dimensionar Hidrantes": só
    ...
    Cálculo". Só é chamado depois que todas as verificações normativas
    passaram (ver imprimir_bloqueio_velocidade/imprimir_bloqueio_hidrante).
    """
    def _montar():
        ...
        _tabela([u"Trecho", u"DN (mm)", u"Q (L/min)", u"V (m/s)", u"Limite (m/s)", u"Verificação"],
                linhas_v,
        ...
        _tabela([u"Hidrante", u"{} (mca)".format(_col_p), u"Q (L/min)",
