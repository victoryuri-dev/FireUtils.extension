...
Lógica de "Inserir linha com limite": desenha sequência de linhas
(clique a clique) até um comprimento máximo. Valida em tempo real
conforme os segmentos são criados. Último segmento é encurtado
automaticamente ao atingir o limite, em vez de descartado.

Detail Line (não Model Line): funciona direto na vista ativa.
"""

from Autodesk.Revit.DB import (
    Line, DetailCurve, Transaction, UnitUtils, BuiltInCategory, GraphicsStyleType
)
...
def inserir_linha_com_limite(doc, uidoc, view, graphics_style, comprimento_max_m):
    """
    Desenha linhas sequencialmente até atingir comprimento máximo.
    Cada clique cria um segmento a partir do ponto anterior.

    Ao ultrapassar o limite máximo:
    - O segmento atual é encurtado para fechar no limite exato
    - O desenho para automaticamente
    - Mostra mensagem com comprimento total inserido

    ESC a qualquer momento termina normalmente.
    """
    limite_interno = _metros_para_interno(comprimento_max_m)

    try:
        ponto_atual = uidoc.Selection.PickPoint(u"Clique o ponto inicial da linha")
    except Exception:
        return
    ...
    while True:
        ...
        try:
            proximo_ponto = uidoc.Selection.PickPoint(
                u"Próximo ponto — {:.2f}m restantes (ESC para terminar)".format(restante_m)
            )
        except Exception:
            if preview_id is not None:
                try:
                    doc.Delete(preview_id)
                except:
                    pass
            break
